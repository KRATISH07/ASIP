...
import {
  ...
  ArrowUpRight, Zap, Shield, Cpu, Waves
} from "lucide-react";
import {
...
const STAT: Record<string, string> = {
  detected: "text-sky-400", analyzing: "text-violet-400", action_planned: "text-cyan-400",
  in_progress: "text-amber-400", resolved: "text-emerald-400", escalated: "text-rose-400",
};
...
export default function Dashboard() {
  ...
  const kpis = [
    ...
    { label: "Active Now", value: data.kpi.active_incidents, delta: "Currently in pipeline", icon: Flame, color: "text-orange-400", accent: "kpi-orange", href: "/incidents" },
    { label: "Critical", value: data.kpi.critical_incidents, delta: "Immediate attention", icon: AlertTriangle, color: "text-rose-400", accent: "kpi-rose", href: "/incidents" },
    { label: "Resolved Today", value: data.kpi.resolved_today, delta: "Avg resolution 42m", icon: CheckCircle2, color: "text-emerald-400", accent: "kpi-emerald", href: "/analytics" },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#09090b] dot-grid">
      ...
      {/* ── Top Status Bar ───────────────────────────────────────────────────── */}
      <div className="border-b border-white/[0.04] px-6 py-2 flex items-center justify-between text-[10px]">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5 text-emerald-400 font-medium">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> All Systems Operational
          </span>
          <span className="text-zinc-700">|</span>
          <span className="text-zinc-500 flex items-center gap-1"><Bot className="w-3 h-3" /> 6 Agents Active</span>
          <span className="text-zinc-700">|</span>
      ...
      <div className="p-5 lg:p-7 space-y-5 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-1">
          ...
          <div className="flex gap-2">
            <Link href="/incidents"
              className="px-3.5 py-2 rounded-lg bg-violet-600 hover:bg-violet-500 text-white text-xs font-semibold transition flex items-center gap-1.5 shadow-lg shadow-violet-600/20">
            ...
            <Link href="/contractors"
              ...
              <HardHat className="w-3.5 h-3.5 text-orange-400" /> Contractors
            </Link>
            <Link href="/analytics"
              className="px-3.5 py-2 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] text-zinc-300 text-xs font-medium border border-white/[0.06] transition flex items-center gap-1.5">
              <TrendingUp className="w-3.5 h-3.5 text-emerald-400" /> Analytics
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {kpis.map((k) => (
            <Link key={k.label} href={k.href} className={`glass-card-clickable ${k.accent} p-5 group block`}>
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-wider">{k.label}</span>
                <k.icon className={`w-4 h-4 ${k.color} opacity-50 group-hover:opacity-100 transition`} />
              </div>
              <p className="text-3xl font-bold text-white tabular-nums">{k.value}</p>
              <p className="text-[10px] text-zinc-600 mt-1.5">{k.delta}</p>
            </Link>
          ))}
        </div>

        {/* ── Chart + Severity + Agent Nodes ─────────────────────────────────── */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">

          {/* Chart - 5 cols */}
          <Link href="/analytics" className="lg:col-span-5 glass-card-clickable p-5 block group">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h2 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
                  <Waves className="w-4 h-4 text-violet-400" /> Incident Wave
                </h2>
                <p className="text-[10px] text-zinc-600 mt-0.5">7-day velocity</p>
              </div>
              <ArrowUpRight className="w-4 h-4 text-zinc-700 group-hover:text-violet-400 transition" />
            </div>
            <div className="h-40">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.incident_trend}>
                  ...
                  <XAxis dataKey="date" stroke="#27272a" fontSize={10} tickLine={false} axisLine={false} />
                  <YAxis stroke="#27272a" fontSize={10} tickLine={false} axisLine={false} width={20} />
                  <Tooltip contentStyle={{ background: "#18181b", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "#e4e4e7", fontSize: 11 }} />
          ...
          {/* Agent Nodes - 4 cols */}
          <Link href="/agent-logs" className="lg:col-span-4 glass-card-clickable p-5 block group">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
                <Bot className="w-4 h-4 text-violet-400" /> Agent Pipeline
              </h2>
              <ArrowUpRight className="w-4 h-4 text-zinc-700 group-hover:text-violet-400 transition" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              {data.agent_activity.map((ag) => (
                <div key={ag.agent_name} className="p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04] hover:border-violet-500/20 transition">
                  <p className="text-[10px] font-semibold text-zinc-300 truncate">{ag.agent_name.replace("Agent", "")}</p>
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-[9px] font-mono text-violet-400">{ag.avg_execution_time_ms}ms</span>
                    <span className={`w-1.5 h-1.5 rounded-full ${ag.success_rate >= 1 ? "bg-emerald-500" : "bg-amber-500"}`} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          ...
          <div className="lg:col-span-2 glass-card p-5 space-y-3">
            ...
            <div className="space-y-1">
              {data.recent_incidents.map((inc) => {
                ...
                return (
                  <div key={inc.id} className="flex items-center justify-between gap-3 p-3 rounded-lg hover:bg-white/[0.03] transition group">
          ...
          <div className="space-y-3">
            ...
            <Link href="/contractors" className="glass-card-clickable p-5 block group space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold text-zinc-200 flex items-center gap-2">
                  <HardHat className="w-4 h-4 text-orange-400" /> Top Contractors
                </h2>
                <ArrowUpRight className="w-4 h-4 text-zinc-700 group-hover:text-violet-400 transition" />
              </div>
              {[
                ...
                <div key={ct.name} className="flex items-center gap-2.5">
                  <div className={`w-6 h-6 rounded-md bg-gradient-to-br ${ct.c} flex items-center justify-center text-[9px] font-bold text-white shadow-sm`}>
                    {i + 1}
                  </div>
            ...
            <div className="glass-card p-4 space-y-0.5">
              <p className="text-[10px] font-medium text-zinc-600 uppercase tracking-wider mb-2 px-2">Quick Access</p>
              {[
                { label: "Sensor Telemetry", href: "/sensor-buffer", dot: "bg-sky-500" },
                { label: "Resident Complaints", href: "/complaints", dot: "bg-violet-500" },
                { label: "Agent Activity Logs", href: "/agent-logs", dot: "bg-emerald-500" },
                { label: "Broadcast Alerts", href: "/notifications", dot: "bg-amber-500" },
                { label: "System Settings", href: "/settings", dot: "bg-zinc-500" },
              ].map((lk) => (
                <Link key={lk.href} href={lk.href}
                  ...
                  <span className="flex items-center gap-2">
                    <span className={`w-1.5 h-1.5 rounded-full ${lk.dot}`} />
                    {lk.label}
                  </span>
